...
class CustomerController extends Controller
{
    ...
    public function show(TmDataPelanggan $pelanggan)
    {
        ...
        $favoriteProducts = $pelanggan->penjualan()
            ->where('status_transaksi', 'selesai')
            ->with('detailPenjualan.produk')
            ->get()
            ->pluck('detailPenjualan')
            ->flatten()
            ->groupBy('produk_id')
            ->map(function ($details) {
                $firstDetail = $details->first();
                return [
                    'produk' => $firstDetail->produk ? $firstDetail->produk->nama_produk : 'Produk Tidak Ditemukan',
                    'total_quantity' => $details->sum('jumlah_beli'),
                    'total_value' => $details->sum('subtotal'),
                    'total_value_format' => 'Rp ' . number_format($details->sum('subtotal'), 0, ',', '.'),
                ];
            })
     ...
     */
    public function destroy(TmDataPelanggan $pelanggan)
    {
        try {
            // Check if customer has transactions
            if ($pelanggan->penjualan()->exists()) {
                return back()->withErrors([
                    'error' => 'Pelanggan tidak dapat dihapus karena memiliki data transaksi'
                ]);
            }
    ...
    public function bulkAction(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'customer_ids.*' => 'exists:tm_data_pelanggan,id',
        ]);
        ...
        try {
            $customers = TmDataPelanggan::whereIn('id', $request->customer_ids);
            $count = $customers->count();

            switch ($request->action) {
                case 'activate':
                    $customers->update(['status_aktif' => true]);
                    $message = "{$count} pelanggan berhasil diaktifkan";
                    break;
                
                case 'deactivate':
                    $customers->update(['status_aktif' => false]);
                    $message = "{$count} pelanggan berhasil dinonaktifkan";
                    break;
                
                case 'delete':
                    // Check if any customer has transactions
                    $customersWithTransactions = $customers->whereHas('penjualan')->count();
                    if ($customersWithTransactions > 0) {
                        return back()->withErrors([
                            'error' => "{$customersWithTransactions} pelanggan tidak dapat dihapus karena memiliki data transaksi"
                        ]);
                    }
                    
                    $customers->delete();
                    $message = "{$count} pelanggan berhasil dihapus";
                    break;
            }
